            ...
            children=html.Label(
                children=[
                    ...
                    dcc.Dropdown(
                        ...
                        options=[
                            {"label": i, "value": i}
                            for i in list(self.parameters["ENSEMBLE"].unique())
                        ],
                        clearable=False,
    ...
    def set_callbacks(self, app: dash.Dash) -> None:
        ...
        def _render_tornado(
            ...
            if ctx in ["", self.tornadoplot.high_low_storage_id] or reset_click:
                if historical_vector(vector, self.smry_meta, True) in self.smry.columns:
                    ...
                    data = filter_ensemble(
                        self.smry, self.parameters, ensemble, [vector]
                    )
...
def filter_ensemble(
    ...
    return pd.merge(
        smry.loc[smry["ENSEMBLE"] == ensemble][smry_columns],
        parameters.loc[parameters["ENSEMBLE"] == ensemble][
            ["REAL", "SENSCASE", "SENSNAME", "SENSTYPE"]
        ],
        on=["REAL"],
    )
